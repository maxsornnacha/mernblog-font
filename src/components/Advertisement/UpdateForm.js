...
const UpdateForm=()=>{
    ...
    const fetchData=()=>{
        axios.get(`${process.env.REACT_APP_API}/ads/single/${id}`)
        .then((response)=>{
            setDetail((prev)=>{
                return ({
    ...
    const Submit=(event)=>{
        ...
        axios.put(`${process.env.REACT_APP_API}/editAds`,{topic,description,adsImg,id},{
            headers:{
    ...
    return(
        ...
        <div className="container p-5 pt-3">
         <h2 className="mt-3 mb-3" style={{textAlign:'center'}}><strong>แก้ไขโฆษณา</strong></h2>
    
    {loadingPage?
    (
        <div>
        <div style={{display:'flex',flexDirection:'column',minHeight:'60vh',justifyContent:'center',alignItems:'center'}}>
